...
import { fetchAlbumsFromDatabase } from '../../mongodb/fetch/database.js';
import client from '../../redis/connect.js';
const transformAlbum = (albums) => {
...
const albumsResolver = async (_, args) => {
...
const albumByIdResolver = async (_, { _id }) => {
...
const albumsByGenreResolver = async (_, { genre }) => {
    if (!genre.trim()) {
...
const addAlbumResolver = async (_, { title, releaseDate, genre, artistId, companyId }) => {
...
const editAlbumResolver = async (_, { _id, title, releaseDate, genre, artistId, companyId }) => {
    const album = await Album.findById(_id);
    if (!album) {
    ...
    if (title)
        album.title = title;
    if (releaseDate && /^(0?[1-9]|1[012])\/(0?[1-9]|[12][0-9]|3[01])\/(19|20)\d\d$/.test(releaseDate))
        album.releaseDate = new Date(releaseDate);
    if (genre)
        album.genre = genre.toUpperCase();
    if (artistId)
        album.artistId = new Schema.Types.ObjectId(artistId);
    if (companyId)
        album.recordCompanyId = new Schema.Types.ObjectId(companyId);
    await album.save();
    const transformedAlbum = transformAlbum([album])[0];
    const cacheKey = `album:${_id}`;
    await client.set(cacheKey, JSON.stringify(transformedAlbum), { EX: 3600 });
...
const removeAlbumResolver = async (_, { _id }) => {
...
export { albumsResolver, albumByIdResolver, albumsByGenreResolver, albumTypeResolver, addAlbumResolver, editAlbumResolver, removeAlbumResolver };
