...
const addArtistResolver = async (_, { name, dateFormed, members }) => {
...
const editArtistResolver = async (_, { _id, name, dateFormed, members }) => {
...
const removeArtistResolver = async (_, { _id }) => {
    const artist = await Artist.findById(_id);
    if (!artist) {
        throw new Error('Artist not found.');
    }
...
export { artistsResolver, artistByIdResolver, searchArtistByArtistNameResolver, artistsTypeResolver, addArtistResolver, editArtistResolver, removeArtistResolver };
