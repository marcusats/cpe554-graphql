...
import type { QueryResolvers, Resolvers, SongResolvers, MutationResolvers } from "../types";  
import { Schema } from 'mongoose';
import client from "../../redis/connect";


const transformSongs = (songs: any[]): Resolvers['Song'][] => {
...
const getSongByIdResolver: QueryResolvers['getSongById'] = async (_, { _id }) => {
    const cacheKey = `song:${_id}`;
    try {
      const cachedSong = await client.get(cacheKey);
      if (cachedSong) return JSON.parse(cachedSong);
...
const getSongsByAlbumIdResolver:  QueryResolvers['getSongsByAlbumId'] = async (_: any, { _id }) => {
    ...
    try {
        ...
        if (!songs) throw new Error('Song not found');
...
const searchSongByTitleResolver: QueryResolvers['searchSongByTitle'] = async (_, { searchTitleTerm }) => {
    ...
    try {
      ...
      if (cachedSongs) return JSON.parse(cachedSongs);
      ...
      if (!songs) throw new Error('No songs found matching search term.');
...
const songTypeResolver: SongResolvers = {
    album: async (parent) => {
      ...
      try {
        ...
        if (cachedAlbum) {
          ...
          if (!album) throw new Error('Album not found for the song');
...
const addSongResolver: MutationResolvers['addSong'] = async (_, { title, duration, albumId }) => {
...
const editSongResolver: MutationResolvers['editSong'] = async (_, { _id, title, duration, albumId }, { client }) => {

    const song = await Song.findById(_id);
    if (!song) throw new Error('Song not found.');
  
    if (title && !title.trim()) throw new Error('Title cannot be empty or just spaces.');
    if (duration && !/^\d{2}:\d{2}$/.test(duration)) throw new Error('Invalid duration format. Please use MM:SS format.');
  
    if (albumId) {
      const album = await Album.findById(albumId);
      if (!album) throw new Error('Album not found with the provided ID.');
...
const removeSongResolver: MutationResolvers['removeSong'] = async (_, { _id }) => {
